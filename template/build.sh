...
build_push_image(){
    if [ -z "${1}" ] || [ -z "${2}" ]; then
        ...
        echo "${1} ${2}"
        if [ ! -z "${TAG}" ]; then
            docker buildx build --build-arg OLS_VERSION=${1} --build-arg PHP_VERSION=${2} --file ./Dockerfile --push --tag ${BUILDER}/${REPO}:${TAG} --platform=linux/amd64 .
        else 
            docker buildx build --build-arg OLS_VERSION=${1} --build-arg PHP_VERSION=${2} --file ./Dockerfile --push --tag ${BUILDER}/${REPO}:${1}-${2} --platform=linux/amd64 .
        fi
    fi    
}
